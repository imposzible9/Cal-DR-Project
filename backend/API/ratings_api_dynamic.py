...
def calculate_accuracy_from_rating_change(history_rows, window_days=90):
    ...
    for row in history_rows:
        ...
        # map to side: 'buy', 'sell', or None
        def rating_side(rtext):
            if not rtext:
                return None
            rl = rtext.lower()
            if 'buy' in rl:
        ...
        rating_side_now = rating_side(rating)
        rating_side_prev = rating_side(prev_rating)

        # Skip Neutral/Unknown or if side cannot be determined
        if rating_side_now is None or rating_side_prev is None:
            continue
        ...
        # same-side if both map to same 'buy'/'sell'
        rating_not_changed = (rating_side_now == rating_side_prev)
        is_correct = False

        # Evaluate correctness by side and threshold
        if rating_side_prev == 'sell' and rating_side_now == 'buy':
            is_correct = change_pct >= CHANGE_THRESHOLD
        elif rating_side_prev == 'buy' and rating_side_now == 'sell':
            is_correct = change_pct <= -CHANGE_THRESHOLD
        elif rating_not_changed:
            if rating_side_now == 'buy':
                is_correct = change_pct >= CHANGE_THRESHOLD
            elif rating_side_now == 'sell':
                is_correct = change_pct <= -CHANGE_THRESHOLD
            else:
                continue
...
def save_accuracy_to_db_new(cur, ticker, timestamp, price, price_prev, change_pct, currency, high, low, window_days, accuracy_result, current_daily_rating=None, current_daily_prev=None):
    """
...
def calculate_and_save_accuracy_for_ticker(cur, ticker, timestamp_str, price, change_pct, currency=None, high=None, low=None, window_days=90):
    ...
    try:
        ...
        save_accuracy_to_db_new(cur, ticker, timestamp_str, price, price_prev, calculated_change_pct, currency, high, low, window_days, accuracy_result, current_daily_rating, current_daily_prev)
