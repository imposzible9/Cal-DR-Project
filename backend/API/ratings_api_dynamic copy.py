...
TV_FIELDS = "Recommend.All,Recommend.All|1W,open,close,change,change_abs,high,low,volume,currency"
...
# จำนวนสัญลักษณ์ต่อหนึ่ง request เมื่อใช้ batch multi-symbol (แนะนำ 10-20)
BATCH_GROUP_SIZE = int(os.getenv("BATCH_GROUP_SIZE") or "15")
    ...
    try:
        ...
        try:
            ...
            for col_def in [
                ...
                ("close_price", "REAL"),
                ("open_price", "REAL"),
                # keep legacy columns if absent to preserve older data layout
                ("price", "REAL"),
                ("open", "REAL"),
                ("change_pct", "REAL"),
                ("change_abs", "REAL"),
...
def construct_tv_symbol(ticker: str, name: str, exchange: str, dr_symbol: str):
    ...
    if match:
        ...
        # Normalize common suffixes for numeric exchange codes like '3692.HK' -> '3692'
        # Keep dots for non-numeric tickers (e.g., BRK.B) but strip when left part is numeric
        if '.' in real_ticker:
            left, right = real_ticker.split('.', 1)
            if left.isdigit():
                real_ticker = left
    else:
        if dr_symbol:
...
def get_market_close_thai(market_code: str, ref_thai: datetime) -> datetime:
    ...
    # เพิ่ม offset 30 นาที ให้การดึง history เกิดหลังเวลาปิดตลาด 30 นาที
    close_thai = close_thai + timedelta(minutes=30)
...
async def fetch_single_ticker(client: httpx.AsyncClient, item_data):
    ...
    for attempt in range(3):
        try:
            ...
            rec_daily = None
            rec_weekly = None
            p_open = p_close = p_change_pct = p_change_abs = p_high = p_low = p_currency = None
            ...
            if isinstance(payload, dict):
                ...
                if p_open is None: p_open = _find_key_recursive(payload, "open")
...
async def fetch_multi_tickers_for_history(client: httpx.AsyncClient, items_tv_symbols: list):
    """Fetch multiple tv symbols in one TradingView request and return mapping tv_symbol -> result dict
    Result dict format mirrors fetch_single_ticker_for_history: {"success": True/False, "data": {...}}"""

    if not items_tv_symbols:
        return {}

    params = {
        "symbol": ",".join(items_tv_symbols),
        "fields": TV_FIELDS,
        "no_404": "true",
        "label-product": "popup-technicals",
    }
    ...
    for attempt in range(3):
        try:
            ...
            results = {tv: {"success": False, "error": "no_data"} for tv in items_tv_symbols}

            # Case A: payload is mapping of tv_symbol -> data
            if isinstance(payload, dict):
                # If payload returns symbol keys directly
                for tv in items_tv_symbols:
                    if tv in payload and isinstance(payload[tv], dict):
                        p = payload[tv]
                    elif "data" in payload and isinstance(payload["data"], dict) and tv in payload["data"]:
                        p = payload["data"][tv]
                    else:
                        p = None

                    if p is None:
                        # Try to extract using recursive find
                        # Some multi responses may nest symbol objects - try to find a matching block
                        p = _find_key_recursive(payload, tv) if tv else None

                    if p is None:
                        results[tv] = {"success": False, "error": "no_data"}
                        continue

                    # Extract fields similarly to single fetch
                    rec_daily = None
                    rec_weekly = None
                    p_open = p_close = p_change_pct = p_change_abs = p_high = p_low = p_currency = None

                    if isinstance(p, dict):
                        rec_daily = p.get("Recommend.All")
                        rec_weekly = p.get("Recommend.All|1W")
                        p_open = p.get("open")
                        p_close = p.get("close")
                        p_change_pct = p.get("change")
                        p_change_abs = p.get("change_abs")
                        p_high = p.get("high")
                        p_low = p.get("low")
                        p_currency = p.get("currency")
            ...
            # Fallback: unknown structure -> return empty results
            return results

        except Exception as e:
            # On network errors try again after short sleep
            await asyncio.sleep(1)
            continue

    # If we fallthrough
    return {tv: {"success": False, "error": "Max retries exceeded"} for tv in items_tv_symbols}
...
async def background_updater():
    ...
    while True:
        try:
            ...
            # Track cycle start time to measure duration
            cycle_start = datetime.now(bkk_tz)
                    ...
                    try:
                        ...
                        for res in results:
                            ...
                            market_data = {
                                ...
                                "open": new_data.get("market_data", {}).get("open"),
            ...
            # Measure total elapsed time for this cycle
            cycle_end = datetime.now(bkk_tz)
            elapsed = cycle_end - cycle_start
            elapsed_ms = int(elapsed.total_seconds() * 1000)
            print(f"✅ Background update cycle finished. Duration: {str(elapsed)} ({elapsed_ms} ms)")
...
def upsert_history_snapshot(
    ...
    cur.execute(
        """
        SELECT daily_rating, weekly_rating, COALESCE(close_price, price) as close_price
        FROM rating_history
        WHERE ticker=? AND timestamp < ?
        ORDER BY timestamp DESC
        LIMIT 1
    ...
    cur.execute(
        ...
        INSERT INTO rating_history (
            ...
            weekly_val, weekly_rating, weekly_prev, weekly_changed_at,
            exchange, market,
            currency, close_price, open_price, change_pct, change_abs, high, low
        )
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """,
        (
...
async def fetch_market_history(market_code: str):
    ...
    async with httpx.AsyncClient() as client:
        ...
        try:
            ...
            # Process tickers in groups to use multi-symbol requests
            for i in range(0, len(market_tickers), BATCH_GROUP_SIZE):
                chunk = market_tickers[i:i+BATCH_GROUP_SIZE]

                # Build tv_symbol list and mapping back to items
                tv_list = []
                item_map = {}
                for item in chunk:
                    ticker = item.get("u_code")
                    name = item.get("u_name")
                    exchange = item.get("u_exch") or ""
                    dr_sym = item.get("dr_sym") or ""
                    tv = construct_tv_symbol(ticker, name, exchange, dr_sym)
                    tv_list.append(tv)
                    item_map[tv] = item

                # Multi-symbol request
                results_map = await fetch_multi_tickers_for_history(client, tv_list)

                for tv in tv_list:
                    item = item_map.get(tv)
                    ticker = item.get("u_code")
                    exchange = item.get("u_exch") or ""

                    # Check existence for today
                    ...
                    res = results_map.get(tv) if isinstance(results_map, dict) else None

                    if not res or not res.get("success"):
                        # Fallback: try single fetch for this ticker
                        print(f"[History] [{market_code}] Multi-fetch failed for {ticker}, trying single fetch")
                        single_res = await fetch_single_ticker_for_history(client, item)
                        if not single_res.get("success"):
                            print(f"[History] [{market_code}] Failed to fetch {ticker}: {single_res.get('error', 'Unknown error')}")
                            continue
                ...
                # Sleep between groups to reduce load on TradingView
                await asyncio.sleep(BATCH_SLEEP_SECONDS)
...
async def market_scheduler(market_code: str):
    ...
    while True:
        try:
            ...
            # ตรวจสอบฤดูกาล และคำนวณเวลาปิดตลาดของวันนี้ (เพิ่ม offset 30 นาที)
            ...
            # คำนวณเวลาปิดตลาดของวันนี้ และเพิ่ม 30 นาทีตามที่ต้องการ
            today = now_thai.date()
            close_today = datetime.combine(today, close_time, tzinfo=bkk_tz) + timedelta(minutes=30)

            # ถ้าเวลาปิดตลาดของวันนี้ (รวม offset) ผ่านไปแล้ว ให้ใช้ของวันถัดไป
...
def calculate_and_save_accuracy_for_ticker(cur, ticker, timestamp_str, price, change_pct, currency=None, high=None, low=None, window_days=90):
    ...
    try:
        ...
        price_prev = None
        cur.execute("""
            SELECT COALESCE(close_price, price) as close_price
            FROM rating_history
            WHERE ticker=? AND timestamp < ?
            ORDER BY timestamp DESC
            LIMIT 1
        ...
        if prev_row:
            ...
            try:
                price_prev = prev_row["close_price"]
            except Exception:
                try:
        ...
        # Ensure the reported `daily_rating`/`daily_prev` and `weekly_rating`/`weekly_prev`
        # in `rating_accuracy` reflect the timeline at the exact `timestamp_str` we are
        # computing for (i.e., use the values from rating_history for this timestamp).
        try:
            if current_daily_rating is not None:
                accuracy_result.setdefault('daily', {})
                accuracy_result['daily']['rating'] = current_daily_rating
                accuracy_result['daily']['prev'] = current_daily_prev
            if current_weekly_rating is not None:
                accuracy_result.setdefault('weekly', {})
                accuracy_result['weekly']['rating'] = current_weekly_rating
                accuracy_result['weekly']['prev'] = current_weekly_prev
        except Exception:
            pass
    ...
    try:
        ...
        for row in ticker_timestamps:
            ...
            try:
                ...
                cur.execute("""
                    SELECT COALESCE(close_price, price) as close_price, change_pct, currency, high, low
                    FROM rating_history
                    WHERE ticker=? AND timestamp=?
                    LIMIT 1
                """, (ticker, timestamp_str))
